...
for url in urls:
    ...
    temp_df['PropertyName'] = pd.Series(property_name)
    temp_df['PropertyName'] = temp_df['PropertyName'].str.strip()

    temp_df['ShortDesc'] = pd.Series(short_desc)
...
locations = pd.read_csv('data/location_lookup.csv', index_col=False)

df = pd.merge(df, locations, on=['LocationCode', 'LocationCode'], how='left')

df.to_csv('data/property_details.csv', index_col=False)
